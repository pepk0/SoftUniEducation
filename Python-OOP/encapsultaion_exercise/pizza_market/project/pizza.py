...
class Pizza:
    def __init__(self, name: str, dough: Dough,
                 max_number_of_toppings: int) -> None:
        self.name = name
        self.dough = dough
    ...
    def max_number_of_toppings(self) -> int:
        return self.__max_number_toppings

    @max_number_of_toppings.setter
    def max_number_of_toppings(self, max_number_of_toppings: int) -> None:
        if max_number_of_toppings <= 0:
            raise ValueError("The maximum number of toppings cannot "
                             "be less or equal to zero")
        self.__max_number_toppings = max_number_of_toppings

    def add_topping(self, topping: Topping) -> None:
        if self.max_number_of_toppings <= len(self.toppings):
            raise ValueError("Not enough space for another topping")

        self.toppings[topping.topping_type] = self.toppings.get(
            topping.topping_type, 0) + topping.weight
